...
public class WriteAlternativePostActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_write_post_alternative);
    ...
    private void post()
    {
        ...
        String choice1 = ((EditText)findViewById(R.id.editTextChose1)).getText().toString();
        String choice2 = ((EditText)findViewById(R.id.editTextChose2)).getText().toString();

        if (title.length() > 0 && content.length() > 0 && choice1.length() > 0 && choice2.length() > 0){
            WriteInfo writeInfo = new WriteInfo(title, content, user.getUid(), PostType.ALTERNATIVE, choice1, choice2, new Date());
            writer(writeInfo);
            finish();
        }else {
